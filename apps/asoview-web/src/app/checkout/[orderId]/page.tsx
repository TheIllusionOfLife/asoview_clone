...
type Props = {
  ...
  searchParams: Promise<{ fakeMode?: string }>;
};
 ...
 * harness. Provider selection is server-side on the Payment row; the
 * client branches on the intent response, not a query param.
 */
export default async function CheckoutPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      ...
      <CheckoutClient orderId={orderId} fakeMode={fakeMode} />
    </div>
  );
}
